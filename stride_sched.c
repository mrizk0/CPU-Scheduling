...
int dyn_priority_comparer(sim_job_t* lhs, sim_job_t* rhs){

  if (lhs->static_priority == 0 && rhs->static_priority == 0){
  ...
  if (lhs->static_priority == 0){
    return 1;
  }
  if (rhs->static_priority == 0){
    return -1;
  }
  ...
  else if (lhs->dynamic_priority == rhs -> dynamic_priority && lhs -> arrival_time < rhs -> arrival_time){
    return -1;
  }
  else if (lhs->dynamic_priority == rhs -> dynamic_priority && lhs -> arrival_time == rhs -> arrival_time && lhs->remaining_size < rhs->remaining_size){
    return -1;
  }
  else{
    return 1;
...
static sim_sched_acceptance_t aperiodic_job_arrival(void*          state,
  ...
  job->dynamic_priority = 0;
  if (s->busy){
    job->dynamic_priority = s->current_job_running->dynamic_priority;
  }
...
static void timer_interrupt(void*          state,
  ...
  if (s->current_job_running->static_priority != 0){
  s->current_job_running->dynamic_priority += (1000000/s->current_job_running->static_priority);
  ...
  // add to queue in order of dynamic priority
   sim_job_queue_enqueue_in_order(&context-> aperiodic_queue,
                                    s->current_job_running,
                                    dyn_priority_comparer);}
  else{
    sim_job_set_remaining_size(s->current_job_running, curr_job_remaining_size);
    sim_job_queue_remove(&context -> aperiodic_queue, s->current_job_running);
