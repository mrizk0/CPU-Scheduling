...
//helper function to compare size of jobs, tie breaker is arrival time
int smaller(sim_job_t* lhs, sim_job_t* rhs){
...
static sim_sched_acceptance_t aperiodic_job_arrival(void*          state,
  ...
  // add the job to the queue based on the size of the job
  sim_job_queue_enqueue_in_order(&context->aperiodic_queue, job, smaller);
...
static void job_done(void*          state,
  ...
  //This is where we have to make the determination of what job we should run next
